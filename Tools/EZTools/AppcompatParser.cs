...
public class AppCompatParser : IArtifactParser
{
        ...
        foreach (var file in files)
        {
            ...
            try
            {
                ...
                foreach (var record in records)
                {
                    ...
                    var parsedDt = dict.GetDateTime("LastModifiedTimeUTC");
                    if (parsedDt == null) continue;

                    string dtStr = parsedDt.Value.ToString("o").Replace("+00:00", "Z");

                    string path = dict.GetString("Path");
                    string filename = string.IsNullOrWhiteSpace(path) ? "" : Path.GetFileName(path);
