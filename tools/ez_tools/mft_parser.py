...
def process_mft(input_dir: str, batch_size: int, extension_filter=None, path_filter=None):
    print(f"[MFT] Looking in: {input_dir}")
    if not os.path.exists(input_dir):
        print(f"  [!] MFT input directory does not exist: {input_dir}")
        return

    mft_files = [f for f in os.listdir(input_dir) if f.endswith(".csv") and "_MFTECmd_$MFT_Output" in f]
    if not mft_files:
    ...
    for filename in mft_files:
        ...
        if should_use_batch(full_path, batch_size):
            chunks = pd.read_csv(full_path, chunksize=batch_size, low_memory=False)
        else:
            chunks = [pd.read_csv(full_path, low_memory=False)]
