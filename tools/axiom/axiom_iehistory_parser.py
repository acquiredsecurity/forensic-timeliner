...
def process_axiom_iehistory(axiom_dir: str, batch_size: int, base_dir: str):
    artifact_name = "Axiom_IEHistory"
    print_and_log(f"[{artifact_name}] Scanning for relevant CSVs under: {axiom_dir}")
    
    ie_files = find_artifact_files(axiom_dir, base_dir, artifact_name)
    if not ie_files:
        print_and_log(f"[{artifact_name}] No IE History files found.")
        return

    for file_path in ie_files:
        ...
        try:
            for df in load_csv_with_progress(file_path, batch_size, artifact_name=artifact_name):
                ...
                for _, row in df.iterrows():
                    timestamp = row.get("Accessed Date/Time - UTC+00:00 (M/d/yyyy)", "")
                    ...
                    timeline_row = {
                        ...
                        "DataPath": url,
                        "DataDetails": row.get("Page Title", ""),
                        "EvidencePath": row.get("Browser Source", "") or os.path.relpath(file_path, base_dir)
                    }
