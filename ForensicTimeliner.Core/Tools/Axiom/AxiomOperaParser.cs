...
public class AxiomOperaParser : IArtifactParser
{
        ...
        foreach (var file in files)
        {
            ...
            try
            {
                ...
                foreach (var record in records)
                {
                    ...
                    var parsedDt = dict.GetDateTime("Date Visited Date/Time - UTC+00:00 (M/d/yyyy)");
                    if (parsedDt == null) continue;

                    string dtStr = parsedDt.Value.ToString("o").Replace("+00:00", "Z");

                    var url = dict.GetString("URL").ToLowerInvariant().Trim();
                    ...
                    var title = dict.GetString("Title") ?? "";
